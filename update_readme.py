...
def generate_language_icons_table(icons_base_url):
    ...
    for lang, icon_filename in languages.items():
        ...
        # 언어 이름 없이 아이콘만 포함된 셀을 추가합니다.
        table_content.append(f"<td><img src='{icon_url}' alt='{lang}' title='{lang}' style='width: 30px; height: 30px;'/></td>")
            ...
            if level_dir.exists():
                ...
                for problem_dir in problem_dirs:
                    if problem_dir.is_dir():
                        problem_name = problem_dir.name.split(" ")[0]
                        ...
                        content.append(f"| [{problem_name}]({problem_url}) | {solution_links} |\n")
    content.append("\n</div>\n")
